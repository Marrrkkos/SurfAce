...
public class OceanFollow : MonoBehaviour
{
    ...
    public float _gridSpacing = 1.0f;
    ...
    void Start()
    {
        ...
        MeshFilter mf = oceanTransform.GetComponent<MeshFilter>();
        if (mf != null && mf.sharedMesh != null)
        {
            // Größe = Mesh-Breite * Scale des Objekts
            offsetX = (mf.sharedMesh.bounds.size.x * oceanTransform.lossyScale.x) / 2f;
            offsetZ = (mf.sharedMesh.bounds.size.z * oceanTransform.lossyScale.z) / 2f;
        }
    ...
    void LateUpdate()
    {
        ...
        float snappedX = Mathf.Round(surfBoardTransform.position.x / _gridSpacing) * _gridSpacing;
        float snappedZ = Mathf.Round(surfBoardTransform.position.z / _gridSpacing) * _gridSpacing;
